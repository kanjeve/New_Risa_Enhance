...
これはVScodeでRisa/Asirのコードを書くための拡張機能 "Risa_Enhancers"です。
...
たくさん。
- エラーでない文もエラーとして出力されることがあります。現在修整中ですが、時間がかかりそうです。
- 計算時間がエラーとして出力されます。
...
2025/07/02
目標です。
- ~~TypeScriptから直接ox_toolkitを呼び出して実行する方式を取る。~~ （作ってみたが既存の方法の方が役立つので不要かもしれない）
- ASTを完成させて、利用する。

2025/11/13
- そろそろ使い方をまとめたpdfファイルを作ろうと考えている。
- 意味解析の完成を目指す。（それ以外の箇所は完成と言っても良いかもしれない）
...
- 構文解析機能を強化しました。
- 意味解析機能も強化しました。
- 意味解析を切れるようにしました（構文解析は残ります）。デフォルトではオフにしていますが、デバッグに協力できる方はオンにして情報提供していただけると助かります。

詳しくは CHANGELOG.md に記載されています。こちらをご参照ください。
