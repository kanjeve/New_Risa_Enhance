...
## インストール方法（vsixファイルに不慣れな方向け）

この拡張機能は、インストーラーファイル（`.vsix` ファイル）を使って手動でインストールします。

**最新バージョン**: `risa-enhancers-0.5.1.vsix`
**ファイルの場所**: このリポジトリの `vsix` フォルダ

### 手順

1. **VS Code を起動する**
   Visual Studio Code を立ち上げます。

2. **拡張機能メニューを開く**
   画面左端にあるアイコン群から、四角形が組み合わさったマーク（**Extensions**）をクリックします。
   （ショートカットキー: `Ctrl + Shift + X` / Macなら `Cmd + Shift + X`）

3. **VSIX からのインストールを選択**
   拡張機能メニューの右上にある「…（3点リーダー）」をクリックし、表示されるメニューから **「VSIX からのインストール... (Install from VSIX...)」** を選択します。

4. **ファイルを選択する**
   ファイル選択画面が開くので、ダウンロードした **`risa-enhancers-0.N.N.vsix`** を選択し、「インストール」をクリックします。

5. **完了**
   右下に「インストールが完了しました」という通知が出れば成功です。
   念のため、VS Code を一度再起動してください。


### ⚠️ 注意事項

* **ダブルクリックでは開きません**: `.vsix` ファイルをダブルクリックしてもインストールできない場合があります。必ず VS Code のメニューからインストールしてください。
* **Risa/Asir 本体が必要です**: この拡張機能は「Risa/Asir 言語を書くための補助ツール」です。計算を実行するには、別途 Risa/Asir 本体のインストールが必要です。
* **「信頼」の確認**: 初めてファイルを開く際、「この作成者を信頼しますか？」と聞かれた場合は、**「信頼する」** を選択してください。
